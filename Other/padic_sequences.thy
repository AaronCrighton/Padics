...
locale padic_int_poly = padic_integers
...
proof-
  ...
  then have "X \<oplus>Y = ((c \<oplus> (\<ominus> a)) \<oplus> a) \<ominus> b" 
    by (metis (no_types, lifting) Zp_is_domain a_minus_def assms(1) assms(2) 
        ...
        domain.axioms(1))
  then have "X \<oplus>Y = (c \<oplus> ((\<ominus> a) \<oplus> a)) \<ominus> b" 
    by (simp add: assms(1) assms(3) cring.cring_simprules(3) cring.cring_simprules(7)
        domain.axioms(1))
  then show ?thesis 
...
next
  ...
  have 0: "a \<ominus> b \<in> nonzero Z\<^sub>p" 
    using False assms(1) assms(2) Zp_not_eq_diff_nonzero by metis 
...
proof-
  ...
  have 1: "?X \<in> nonzero Z\<^sub>p" 
    using Zp_not_eq_diff_nonzero assms(1) assms(2) assms(4) by metis 
  ...
  have 4: "ord_Zp ?Z \<ge> min (ord_Zp ?X) (ord_Zp ?Y)" using 0 1 2 3 
    using Z\<^sub>p_def ord_Zp_ultrametric padic_integers_axioms by auto
  then show ?thesis 
    by (metis assms(1) assms(2) min.commute ord_Zp_dist_sym)
qed
...
lemma ord_Zp_dist_res_eq:
  ...
  shows "(a k) = (b k)" 
  using Zp_res_eq assms(1) assms(2) assms(3) 
  by blast
...
lemma ord_Zp_dist_res_eq2:
  ...
  using Zp_res_eq2 assms(1) assms(2) assms(3) assms(4) 
  by blast
...
proof-
  have "ord_Zp_dist b c  \<ge> min (ord_Zp_dist a c) (ord_Zp_dist a b)" 
    using assms ord_Zp_dist_ultrametric  by metis 
  ...
  have 3: "(b \<ominus> c) k = 0"
    using "0" Zp_is_cring assms(2) assms(3) assms(6) cring.cring_simprules(4)[of Z\<^sub>p b c]
      zero_below_ord[of "b \<ominus>c" k] by linarith 
  show "a k = b k" 
    using Zp_res_eq assms(1) assms(2) assms(4) assms(6) 
    by (smt less_imp_le_nat of_nat_mono) 
  show "a k = c k" 
    using Zp_res_eq assms(1) assms(3) assms(5) assms(6) 
    by (smt less_imp_le_nat of_nat_mono)
  show "b k = c k" 
  ...
  proof-
    ...
    next
      ...
      have A1: "s n0 \<in> carrier Z\<^sub>p" 
        using is_cauchy_def assms is_closed_simp
        by blast
      have A2: "s n1 \<in> carrier Z\<^sub>p" 
        using is_cauchy_def assms is_closed_simp 
        by blast
      show ?thesis 
        using A0 ord_Zp_dist_res_eq  A1 A2 by metis 
    qed
  qed
  then show ?thesis 
    using that by blast
...
proof-
  ...
  proof
        ...
        proof
          ...
          next
            ...
            have 1: "(s k) \<in> carrier Z\<^sub>p" 
              using Z\<^sub>p_def assms(1) padic_int_poly.is_closed_simp padic_int_poly_axioms by blast
    ...
    qed
    then show ?thesis 
      using zless_nat_conj zless_nat_eq_int_zless by metis
  qed
  qed
  then show ?thesis 
...
proof-
  obtain N where A0: "\<And> n0 n1. n0 > N \<and> n1 > N \<Longrightarrow> (s n0) n = (s n1) n"
    using assms padic_int_poly.is_cauchy_imp_eventually_const_0 padic_int_poly_axioms by blast
  obtain r where A1: "s (Suc N) n = r" 
    by simp
  have 0: "r \<in> carrier (R n)" 
    using  Z\<^sub>p_def \<open>s (Suc N) n = r\<close> assms is_closed_simp 
      padic_int_poly.is_cauchy_def padic_int_poly_axioms padic_set_simp0 by auto
  ...
  proof-
    ...
    proof-
      obtain N1 where N1_def: "\<forall>n0 n1. N1 < n0 \<and> N1 < n1 \<longrightarrow> s n0 n = s n1 n" 
        using assms(1) padic_int_poly.is_cauchy_imp_eventually_const_0 padic_int_poly_axioms  by blast
      obtain N2 where N2_def: "\<forall>n0 n1. N2 < n0 \<and> N2 < n1 \<longrightarrow> t n0 n = t n1 n" 
        using assms(2) padic_int_poly.is_cauchy_imp_eventually_const_0 padic_int_poly_axioms by blast
...
lemma cauchy_approaches_in_ord:
  assumes "is_cauchy s"
  assumes "a = res_lim s"
  obtains N where "\<And>n. n> N \<Longrightarrow> (s n = a) \<or> ord_Zp (a \<ominus> (s n)) > k"
proof-
  have " (\<forall>n. \<exists>k. \<forall>m>k. s m = a \<or> n \<le> ord_Zp_dist (s m) a)"
    using converges_to_def[of s a] assms is_cauchy_imp_has_limit[of s a] 
    by blast
  then have "\<exists>N. \<forall>m>N. s m = a \<or> 1+ k \<le> ord_Zp_dist (s m) a"
    by blast 
  then obtain N where N_def: "\<forall>m>N. s m = a \<or> 1+ k \<le> ord_Zp_dist (s m) a"
    by blast
  have "\<And>n. n> N \<Longrightarrow> (s n = a) \<or> ord_Zp (a \<ominus> (s n)) > k"
  proof-
    fix m
    assume "m > N"
    then have 0: "s m = a \<or> 1+ k \<le> ord_Zp_dist (s m) a"
      using N_def   
      by (simp add: ord_Zp_def)
    show "s m = a \<or> k < ord_Zp_dist a (s m)"
      apply(cases "s m = a")
      using 0 apply blast
    proof-
      assume " s m \<noteq> a"
      show "s m = a \<or> k < ord_Zp_dist a (s m) "
      proof-
        have 1: " 1+k \<le> ord_Zp_dist (s m) a"
          using "0" \<open>s m \<noteq> a\<close> by blast
        have "k < ord_Zp_dist a (s m) "
        proof-
          have 10: "a \<in> carrier Z\<^sub>p"
            by (simp add: assms(1) assms(2) res_lim_in_Zp)
          have 11: "s m \<in> carrier Z\<^sub>p"
            using assms(1) is_cauchy_def is_closed_seq_def by blast
          have 13: "ord_Zp_dist a (s m) = ord_Zp_dist (s m) a"
            using "10" "11" ord_Zp_dist_sym by blast
          show ?thesis 
            using 1 13 
            by linarith 
        qed
  ...
  qed
  show ?thesis 
    using \<open>\<And>n. N < n \<Longrightarrow> s n = a \<or> k < ord_Zp_dist a (s n)\<close> that by blast
qed
...
proof
  ...
  then show "(f\<^sub>*s) x = (g\<^sub>*s) x" 
    using assms(1) eq_simp padic_int_poly.push_forward_def padic_int_poly_axioms by auto
qed
...
lemma push_forward_of_constant_is_constant:
  ...
  by (metis (full_types) is_closed_simp is_constant_fun_def
      is_constant_seq_def padic_int_poly.push_forward_def padic_int_poly_axioms)
...
lemma push_forward_of_closed_is_closed:
  ...
  using is_closed_fun_def is_closed_seq_def assms(1) assms(2)
    padic_int_poly.push_forward_def padic_int_poly_axioms by auto
...
proof(rule is_continuousI)
  show "is_closed_fun (fun_sum f g)" using assms 
    using Z\<^sub>p_def Zp_is_cring cring.cring_simprules(1) is_continuous_def
      fun_sum_def is_closed_fun_def padic_int_poly_axioms by fastforce
...
proof(rule is_continuousI)
  show "is_closed_fun (fun_prod f g)" 
    using Z\<^sub>p_def Zp_is_cring assms(1) assms(2) cring.cring_simprules(5) 
      ...
      padic_int_poly_axioms by fastforce
...
proof(rule is_cauchyI)
  show "is_closed_seq (alt_seq s)"
    ...
    using Z\<^sub>p_def assms is_closedI 
          is_closed_simp
          padic_int_poly_axioms res_lim_in_Zp 
    by presburger
  fix n
  ...
  proof-
    obtain N where N_def: " \<forall>n0 n1. N < n0 \<and> N < n1 \<longrightarrow> s n0 n =  s n1 n "
      using assms is_cauchy_imp_eventually_const_0
            padic_int_poly_axioms 
      by blast
    ...
    proof-
      ...
      show "alt_seq s n0 n = alt_seq s n1 n"
        ...
        by (smt A(1) A(2) assms lessI max_less_iff_conj 
            res_lim_cauchy padic_int_poly_axioms)
    qed
    then show ?thesis 
      by blast
  qed
...
proof-
  ...
  proof-
    ...
    obtain Na where Na_def: "\<And>m. m > Na \<Longrightarrow> (f\<^sub>*s) m k = a k"
      using a_def assms  is_continuous_def 
            padic_int_poly_axioms res_lim_cauchy
      by blast
    obtain Nb where Nb_def: "\<And>m. m > Nb \<Longrightarrow> (f\<^sub>*t) m k = b k"
      using b_def assms is_continuous_def 
            padic_int_poly_axioms res_lim_cauchy
            alt_seq_cauchy 
      by blast
...
proof-
  ...
  have 0: "converges_to (f\<^sub>*s) (res_lim (f\<^sub>*s))"
    using assms(1) assms(2) is_continuous_def 
      is_cauchy_imp_has_limit padic_int_poly_axioms by blast
  ...
  have 2: "converges_to (f\<^sub>*s) (f (res_lim t))"
    using "1" assms(1) assms(2) res_lim_pushforward padic_int_poly_axioms t_def by auto
...
"is_continuous X_Zp"
  ...
  by (simp add: X_Zp_def is_closed_fun_def padic_int_poly_axioms)
...
proof(induction n)
  case 0
  then show ?case 
    using X_pow_Zp_def Z\<^sub>p_def Zp_one_car is_closed_fun_def
      padic_int_poly_axioms by auto
next
  case (Suc n)
  then show ?case 
...
proof(rule is_closed_funI)
  ...
  then show " scalar_mult c f x \<in> carrier Z\<^sub>p" 
    ...
    by (simp add: cring.cring_simprules(5) is_closed_fun_def)
qed
...
proof
  ...
  show "scalar_mult c f x = fun_prod (to_const_fun c) f x" 
    using fun_prod_def to_const_fun_def padic_int_poly_axioms scalar_mult_def by auto
qed
