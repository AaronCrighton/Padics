...
abbreviation Zp_x where
"Zp_x \<equiv> UP Z\<^sub>p"

lemma Zp_x_is_UP_ring[simp]:
"UP_ring Z\<^sub>p"
  by (metis UP_ring.intro Zp_is_cring cring_def)

lemma Zp_x_is_UP_cring[simp]:
"UP_cring Z\<^sub>p"
  by (simp add: UP_cring_def)

lemma Zp_x_is_UP_domain[simp]:
"UP_domain Z\<^sub>p"
  by (simp add: UP_domain_def Zp_is_domain)

lemma Zp_x_ring[simp]:
"ring Zp_x "
  by (simp add: UP_ring.UP_ring)

lemma Zp_x_cring[simp]:
"cring Zp_x "
  by (simp add: UP_cring.UP_cring)

lemma Zp_x_domain[simp]:
"domain Zp_x "
...
lemma pow_closedP[simp]:
  ...
  shows "f[^]\<^bsub>Zp_x\<^esub>(n::nat) \<in> carrier Zp_x " 
  by (meson Zp_x_ring assms monoid.nat_pow_closed ring_def)
...
lemma pow_zeroP[simp]:
  assumes "n >0"
  shows "\<zero>\<^bsub>Zp_x\<^esub>[^]\<^bsub>Zp_x\<^esub>(n::nat) = \<zero>\<^bsub>Zp_x\<^esub>"
  using assms ring.pow_zero[of Zp_x] by simp

lemma sum_closedP[simp]:
  assumes "f \<in> carrier Zp_x"
  assumes "g \<in> carrier Zp_x"
  shows "f \<oplus>\<^bsub>Zp_x\<^esub>g \<in>  carrier Zp_x"
...
lemma mult_zeroP[simp]:
  assumes "f \<in> carrier Zp_x" 
  shows "f \<otimes>\<^bsub>Zp_x\<^esub> \<zero>\<^bsub>Zp_x\<^esub> = \<zero>\<^bsub>Zp_x\<^esub>"
        "\<zero>\<^bsub>Zp_x\<^esub> \<otimes>\<^bsub>Zp_x\<^esub> f = \<zero>\<^bsub>Zp_x\<^esub>"
  apply (simp add: assms cring.cring_simprules(27))
  apply (simp add: assms cring.cring_simprules(26))
  done
...
lemma add_zeroP[simp]:
  assumes "f \<in> carrier Zp_x "
  shows "f \<oplus>\<^bsub>Zp_x\<^esub> \<zero>\<^bsub>Zp_x\<^esub> = f"
        "\<zero>\<^bsub>Zp_x\<^esub> \<oplus>\<^bsub>Zp_x\<^esub> f = f"
   apply (simp add: assms cring.cring_simprules)
   apply (simp add: assms cring.cring_simprules)
   done
...
(*Zp_x addition commutes with evaluation addition*)
...
proof-
  ...
  proof-
    ...
    then show "eval Z\<^sub>p Z\<^sub>p (\<lambda>x. x) x f \<oplus> eval Z\<^sub>p Z\<^sub>p (\<lambda>x. x) x g = eval Z\<^sub>p Z\<^sub>p (\<lambda>x. x) x h "
      using assms ring_hom_add   by (simp add: \<open>f \<in> carrier Zp_x\<close> ring_hom_def)
  qed
qed
...
proof-
  have 1: "f\<^emph>x = c \<otimes> x [^] n"
    using UP_domain.fun_of_monom assms Zp_x_is_UP_domain 
  ...
  proof-
    ...
    proof-
      ...
      have C3: "f\<^emph>x = (\<Oplus>i \<in> {..degree f}. (cf f i) \<otimes> x [^] i)" 
        using  A  C2 Zp_x_is_UP_domain UP_domain.fun_of_formula[of Z\<^sub>p f x]  
        by blast
      show ?thesis 
...
lemma monom_poly_is_continuous':
  assumes "is_monom f"
  shows "is_continuous (fun_of f)"
  using Zp_x_is_UP_domain UP_domain.is_monom_id[of Z\<^sub>p f]
...
proof- 
  obtain c where c0: "c \<in> carrier Z\<^sub>p" and  c1: "\<And>x.  x \<in> carrier Z\<^sub>p \<Longrightarrow> (fun_of f) x = c"
    using Zp_x_is_UP_domain UP_domain.degree_0_imp_constant0  assms(1) assms(2) by blast
  ...
  proof-
    ...
    next
      ...
      obtain g where g_def: "g \<in> carrier Zp_x \<and> (f = g \<oplus>\<^bsub>Zp_x\<^esub> (lt f)) \<and> degree g < degree f"
        using E0 A0 Zp_x_is_UP_domain UP_domain.lt_decomp[of Z\<^sub>p f] by auto  
...
"to_poly \<in> ring_hom Z\<^sub>p Zp_x"
...
"X_pow n \<equiv> X[^]\<^bsub>Zp_x\<^esub>n"
...
proof(induction n)
  case 0
  then show ?case 
    by (smt Zp_x_is_UP_domain Zp_x_is_UP_ring Zp_x_ring UP_domain.X_is_poly UP_ring.deg_monom 
...
next
  case (Suc n)
  fix n::nat
  assume IH: "(X [^]\<^bsub>Zp_x\<^esub> n \<^emph> a) k = (X [^]\<^bsub>Zp_x\<^esub> n \<^emph> b) k"
  show "(X [^]\<^bsub>Zp_x\<^esub> Suc n \<^emph> a) k = (X [^]\<^bsub>Zp_x\<^esub> Suc n \<^emph> b) k"
  proof-
    have LHS0: "(X [^]\<^bsub>Zp_x\<^esub> Suc n \<^emph> a) k = ((X [^]\<^bsub>Zp_x\<^esub> n \<^emph> a) \<otimes> (X \<^emph> a)) k"
      by (metis (no_types, lifting) Zp_x_is_UP_domain Zp_x_ring UP_domain.X_is_poly
          UP_domain.fun_of_mult assms(1) monoid.nat_pow_Suc pow_closedP ring_def)
    have LHS1: "(X [^]\<^bsub>Zp_x\<^esub> Suc n \<^emph> a) k = (X [^]\<^bsub>Zp_x\<^esub> n \<^emph> a) k \<otimes>\<^bsub>R k\<^esub> (X \<^emph> a) k"
      using LHS0 
      by (simp add: Res_def Z\<^sub>p_def padic_mult_simp)
    then show ?thesis using assms IH 
      by (smt Zp_x_is_UP_domain Zp_x_ring UP_domain.X_is_poly 
...
proof-
  have "lt f = (lc f)\<odot>\<^bsub>Zp_x\<^esub>(X_pow (degree f))"
    by (simp add: UP_domain.lt_rep_X_pow assms(3))
  have LHS0: "(fun_of (lt f) a) = (lc f) \<otimes> (fun_of (X_pow (degree f)) a)"
    by (simp add: UP_domain.X_is_poly UP_domain.fun_of_smult UP_domain.lc_closed \<open>lt f = lc f \<odot>\<^bsub>Zp_x\<^esub> X [^]\<^bsub>Zp_x\<^esub> degree f\<close> assms(1) assms(3))
  have RHS0: "(fun_of (lt f) b) = (lc f) \<otimes> (fun_of (X_pow (degree f)) b)"
    by (simp add: UP_domain.X_is_poly UP_domain.fun_of_smult UP_domain.lc_closed \<open>lt f = lc f \<odot>\<^bsub>Zp_x\<^esub> X [^]\<^bsub>Zp_x\<^esub> degree f\<close> assms(2) assms(3))
...
proof(rule UP_domain.poly_induct2[of Z\<^sub>p f])
  show "UP_domain Z\<^sub>p" 
    by simp
  show "f \<in> carrier Zp_x" 
    using assms by simp
  show "\<And>p. p \<in> carrier Zp_x \<Longrightarrow> degree p = 0 \<Longrightarrow> (p \<^emph> a) k = (p \<^emph> b) k"
    by (metis Zp_x_is_UP_domain UP_domain.degree_0_imp_constant0 assms(2) assms(3))
  show "\<And>p. 0 < degree p \<Longrightarrow> p \<in> carrier Zp_x \<Longrightarrow> (trunc p \<^emph> a) k = (trunc p \<^emph> b) k 
                \<Longrightarrow> (p \<^emph> a) k = (p \<^emph> b) k"
  proof-
    fix p
    assume A: "0 < degree p" "p \<in> carrier Zp_x"
    assume IH: " (trunc p \<^emph> a) k = (trunc p \<^emph> b) k "
    show "(p \<^emph> a) k = (p \<^emph> b) k"
    proof-
      have "(p \<^emph> a) k = (((trunc p)\<^emph> a) \<oplus> ((lt p) \<^emph> a)) k "
        by (metis A(2) Zp_x_is_UP_domain UP_domain.fun_of_plus 
      ...
      then have 2: "(p \<^emph> a) k = (((trunc p) \<oplus>\<^bsub>Zp_x\<^esub>(lt p)) \<^emph> b) k"
        by (simp add: A(2) UP_domain.fun_of_plus UP_domain.lt_closed 
            UP_domain.trunc_simps(3) assms(3))
      then show ?thesis 
        by (metis A(2) Zp_x_is_UP_domain UP_domain.trunc_simps(1))
    qed
  qed
qed
...
proof-
  ...
  have 2: "pderiv f \<in> carrier Zp_x"
...
"hensel f a \<equiv> (f \<in> carrier Zp_x \<and> a \<in> carrier Z\<^sub>p \<and> ((f \<^emph> a) 1) = 0 \<and> (deriv f a 1) \<noteq> 0)"

(*Proving the existence of the sequence for hensel's lemma*)
lemma pre_hensel:
assumes "hensel f a"
assumes "b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc n) = 0 \<and> b 1 = a 1"
shows "\<exists> b' \<in> carrier Z\<^sub>p. (f \<^emph> b') (Suc (Suc n)) = 0 \<and> b' 1 = a 1 \<and> b' (Suc n) = b (Suc n)"
proof-
    have b_def: "b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc n) = 0 \<and> b 1 = a 1"
      using assms by blast 
    have  "val_Zp (\<p>[^](Suc n)) \<preceq>\<^bsub>G\<^esub> val_Zp (f \<^emph> b)"
    proof(cases "f \<^emph> b = \<zero>")
      case True
      then show ?thesis 
        by (simp add: G_ord(1) val_Zp_def)
    next
      case False
      then have "f \<^emph> b \<in>nonzero Z\<^sub>p"
        using assms(1) b_def not_nonzero_Zp by auto
      then have "ord_Zp (f \<^emph> b) \<ge> Suc n" 
        using b_def  False Zp_nonzero_def(1) ord_Zp_geq by force
      then have "ord_Zp (\<p>[^](Suc n)) \<le> ord_Zp (f \<^emph> b)" 
        using b_def  by (simp add: ord_Zp_p_pow)
      then show ?thesis 
        using False G_ord(3) \<open>int (Suc n) \<le> ord_Zp (f \<^emph> b)\<close> ord_Zp_def val_Zp_def val_Zp_p_pow by auto
    qed
    then have 1:"divide (f \<^emph> b) (\<p>[^](Suc n)) \<otimes> (\<p>[^](Suc n)) = f \<^emph> b"
    ...
    have 3: "(deriv f a) 1 \<noteq> 0" 
      using assms by blast
    have "ord_Zp (deriv f a) = 0" 
      using 2 3 by (simp )
    then have 4: "(deriv f a) \<in> Units Z\<^sub>p" 
      using 2 ord_Zp_0_imp_unit[of "(deriv f a)"] 
      by blast
    
    obtain k where  k_def: "k =inv\<^bsub>(R 1)\<^esub> ((deriv f a) 1)" 
      by simp
    ...
    proof-
      ...
      have P3: "k \<in> carrier (R 1)" 
        using k_def P0 P1 P field.field_inv[of "R 1" "(deriv f a) 1"] by auto 
      then show ?thesis 
        using P3 P2 by auto 
    qed
    
    obtain m::int where m_def: "m =(\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<otimes>\<^bsub>R 1\<^esub> k))"
      by simp
    obtain t where t_def: "t = [m]\<cdot>\<one>"
      by simp
   
    have 5: "(f \<^emph> (b \<oplus> ((\<p>[^](Suc n)) \<otimes> t))) (Suc (Suc n)) = 0"
    proof-
      obtain fb where fb_def[simp]: "fb = f \<^emph> b"
        by simp
      obtain f'b where f'b_def[simp]: "f'b = (deriv f b)"
        by simp
      obtain c where c_def: "c = (shift 2 (T\<^bsub>b\<^esub>f)) \<^emph> ((\<p>[^](Suc n)) \<otimes> t)"
        by simp
      ...
      have c_car:"c \<in> carrier Z\<^sub>p"
        using Zp_x_is_UP_domain Taylor_closed_UP_domain 
              poly_shift_closed_UP_domain pnt_car c_def
        by (simp add: assms)
      have A0: " f \<^emph> (\<p> [^] (Suc n) \<otimes> t \<oplus> b) = fb \<oplus> f'b \<otimes> (\<p> [^] (Suc n) \<otimes> t) \<oplus> c \<otimes> (\<p> [^](Suc n) \<otimes> t) [^] (2::nat)"
      proof-
        have P0: "\<p> [^] (Suc n) \<otimes> t \<in> carrier Z\<^sub>p"
          by (simp add: Zp_int_mult_closed Zp_one_car cring.cring_simprules(5)  t_def)
        have P1: "f \<in> carrier Zp_x"
          using assms by auto 
        have P2: "b \<in> carrier Z\<^sub>p"
          using b_def by auto 
        show ?thesis
          using P0 P1 P2   fb_def f'b_def c_def  
                Zp_x_is_UP_domain UP_domain.Taylor_deg_1_eval[of Z\<^sub>p f b "(\<p>[^](Suc n)) \<otimes> t" c fb f'b]
        by auto 
      qed
      ...
      proof-
        have A20: "f'b \<in> carrier Z\<^sub>p"
          using f'b_def  assms b_def UP_domain.derivative_closed[of Z\<^sub>p f b] Zp_x_is_UP_domain  
          by blast
      ...
      have A4: " (f \<^emph> (\<p> [^] (Suc n) \<otimes> t \<oplus> b)) (Suc (Suc n)) 
                    = (fb \<oplus> f'b \<otimes> (\<p> [^] (Suc n) \<otimes> t)) (Suc (Suc n))"
        using A0 A1 A2 A3
      ...
      proof-
        ...
        proof-
          ...
          proof-
            have C10:"(f'a \<otimes> t) 1  = (f'a 1) \<otimes>\<^bsub>R 1\<^esub> (t 1)"
              by (simp add: Res_def Z\<^sub>p_def padic_mult_simp)
            have C11:"(f'a \<otimes> t) 1  = (f'a 1) \<otimes>\<^bsub>R 1\<^esub>(\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<otimes>\<^bsub>R 1\<^esub> k))"
              using C10 m_def C0 
            ...
            have C12: "(f'a \<otimes> t) 1  = ((f'a 1) )\<otimes>\<^bsub>R 1\<^esub>(\<ominus>\<^bsub>R 1\<^esub> (k \<otimes>\<^bsub>R 1\<^esub>(((divide (f \<^emph> b) (\<p>[^](Suc n))) 1))))"
            proof-
              have C120:"((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<in> carrier (R 1)"
                by (metis A7 Res_def Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> 
              ...
              have C122: "(\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<otimes>\<^bsub>R 1\<^esub> k))
                              =(\<ominus>\<^bsub>R 1\<^esub> (k \<otimes>\<^bsub>R 1\<^esub> ((divide (f \<^emph> b) (\<p>[^](Suc n))) 1)))"
            ...
            have C14: "f'a 1 \<in> carrier (R 1)"
              by (metis "2" Res_def Z\<^sub>p_def f'a_def padic_set_simp0 partial_object.select_convs(1))
            have C15: "(k \<otimes>\<^bsub>R 1\<^esub>(((divide (f \<^emph> b) (\<p>[^](Suc n))) 1))) \<in> carrier (R 1)"
            proof-
              have "(divide (f \<^emph> b) (\<p>[^](Suc n))) \<in> carrier Z\<^sub>p"
                using A7 \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> divide_id'(1) fb_def p_pow_nonzero 
                by blast
              then have "(divide (f \<^emph> b) (\<p>[^](Suc n))) 1 \<in> carrier (R 1)"
                by (metis Res_def Z\<^sub>p_def padic_set_simp0 partial_object.select_convs(1))
              then show ?thesis using Res_1_field C13 
                by simp
            qed
            then have "(f'a \<otimes> t) 1  = (\<ominus>\<^bsub>R 1\<^esub> ((f'a 1) )\<otimes>\<^bsub>R 1\<^esub>(k \<otimes>\<^bsub>R 1\<^esub>(((divide (f \<^emph> b) (\<p>[^](Suc n))) 1))))"
            ...
            then have "(f'a \<otimes> t) 1  = (\<ominus>\<^bsub>R 1\<^esub> ((f'a 1) \<otimes>\<^bsub>R 1\<^esub> k ) \<otimes>\<^bsub>R 1\<^esub>(((divide (f \<^emph> b) (\<p>[^](Suc n))) 1)))"
              using C12 C13 C14 C15 Res_1_field R_cring[of 1] R_residues[of 1]
              unfolding Res_def 
              by (metis (no_types, lifting) A7 Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> cring.cring_simprules(11) cring.cring_simprules(28) cring.cring_simprules(3) divide_id'(1) fb_def p_pow_nonzero padic_set_simp0 partial_object.select_convs(1) zero_less_one)
            then have  "(f'a \<otimes> t) 1  = (\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1)))"
              using k_def 4 Res_1_field  C12 C13 C14 C15 Res_1_field R_cring[of 1] R_residues[of 1]
              unfolding Res_def 
              by (smt A7 Res_def Z\<^sub>p_def \<open>k \<otimes>\<^bsub>R 1\<^esub> deriv f a 1 = \<one>\<^bsub>R 1\<^esub> \<and> k \<in> carrier R 1\<close> \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> cring.cring_simprules(12) cring.cring_simprules(14) cring.cring_simprules(29) cring.cring_simprules(3) cring.cring_simprules(5) divide_id'(1) f'a_def fb_def p_pow_nonzero padic_set_simp0 partial_object.select_convs(1) zero_less_one)
            then show ?thesis
        ...
        then have A81: "(f'a \<otimes> t) 1= (\<ominus> (divide (fb) (\<p>[^](Suc n)))) 1"
          using res_uminus  
          using A7 Res_def Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> 
            divide_id'(1) p_pow_nonzero padic_inv prime by auto
        have A82: "(fb \<oplus> f'a \<otimes>(\<p>[^](Suc n) \<otimes> t)) (Suc (Suc n)) = 0"
        proof-
          obtain h where h_def: "h \<in> carrier Z\<^sub>p \<and> (f'a \<otimes> t) 
                          = (\<ominus>(divide (fb) (\<p>[^](Suc n)))) \<oplus> h \<otimes> \<p>[^](1::nat)"
            ...
            by (metis (no_types, lifting) "2" A7 A81 Zp_int_inc_closed Zp_is_cring 
                \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> cring.cring_simprules(14)
          ...
          have A820: "(f'a \<otimes> t) \<in> carrier Z\<^sub>p"
            by (metis A7 Zp_is_cring \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close>
          ...
          have A823:"(f'a \<otimes> t) \<oplus> (divide (fb) (\<p>[^](Suc n))) =  h \<otimes> \<p>[^](1::nat)"
            ...
            by (metis A7 \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close>
          ...
          proof-
            ...
            then have A8240: "((f'a \<otimes> t) \<otimes>  (\<p>[^](Suc n))) \<oplus> (divide (fb) (\<p>[^](Suc n))) \<otimes>  (\<p>[^](Suc n)) =  h \<otimes> (\<p>[^](1::nat) \<otimes>  (\<p>[^](Suc n)))"
              ...
              by (metis A7 \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> cring.cring_simprules(11)
        ...
        qed
        have A83: "(f'a \<otimes>(\<p>[^](Suc n) \<otimes> t)) (Suc (Suc n))
        ...
        proof- 
          ...
          have A832: "f'b \<in> carrier Z\<^sub>p"
            using f'b_def b_def assms UP_domain.derivative_closed[of Z\<^sub>p f b] Zp_x_is_UP_domain 
            by auto 
    ...
    have b11: "(f \<^emph> b1) (Suc (Suc n)) = 0"
      using b1_def 5 
      by auto
    have b12: "b1 1 = a 1"
    proof-
    ...
    show ?thesis 
      using b10 b11 b12 b13 by blast
qed

(*The sequence which will converge to the root obtained by hensel's lemma*)
fun hensel_seq where
"hensel_seq f a 0 = a"|
"hensel_seq f a (Suc n) = (SOME b. b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc (Suc n)) = 0 \<and> b 1 = a 1 \<and> b (Suc n) = (hensel_seq f a n) (Suc n))"
...
lemma hensel_seq_id_induct:
 ...
 "\<And>b0 b1. b0  = hensel_seq f a n \<Longrightarrow>
  ...
  b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc n)) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc n) = b0 (Suc n)"
proof(induction n)
  show "\<And>b0 b1. b0 = hensel_seq f a 0 \<Longrightarrow> 
        b1 = hensel_seq f a (Suc 0) \<Longrightarrow> 
        b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc 0)) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc 0) = b0 (Suc 0)"
  proof-
    fix b0 b1
    ...
    have B0: "b0 = a" 
      using B 
      by simp
    have B1: "b0 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b0) (Suc 0) = 0 \<and> b0 1 = a 1"
      using assms B0 
      by auto 
    have B2: "\<exists> b' \<in> carrier Z\<^sub>p. (f \<^emph> b') (Suc (Suc 0)) = 0 \<and> b' 1 = a 1 \<and> b' (Suc 0) = b0 (Suc 0)"
      using pre_hensel assms B1 
      by blast
    have B3: "b1= (SOME b. b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc (Suc 0)) = 0 \<and> b 1 = a 1 \<and> b (Suc 0) = b0 (Suc 0))"
      using B
      by simp
    show  "b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc 0)) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc 0) = b0 (Suc 0)"
      using B3 B2 
  ...
  show "\<And>n b0 b1. (\<And>b0 b1. b0 = hensel_seq f a n \<Longrightarrow> b1 = hensel_seq f a (Suc n) \<Longrightarrow>
             b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc n)) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc n) = b0 (Suc n)) \<Longrightarrow>
             b0 = hensel_seq f a (Suc n) \<Longrightarrow>
             b1 = hensel_seq f a (Suc (Suc n)) \<Longrightarrow> 
             b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc (Suc n))) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc (Suc n)) = b0 (Suc (Suc n))"
  proof-
    fix n b0 b1
    ...
    show "b1 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b1) (Suc (Suc (Suc n))) = 0 \<and> b1 1 = a 1 \<and> b1 (Suc (Suc n)) = b0 (Suc (Suc n))"
    proof-
      have A0: " b0 \<in> carrier Z\<^sub>p \<and> (f \<^emph> b0) (Suc (Suc n)) = 0 \<and> b0 1 = a 1 \<and> b0 (Suc n) = (hensel_seq f a n) (Suc n)"
        using A IH 
        by blast
      have A1: "\<exists> b' \<in> carrier Z\<^sub>p. (f \<^emph> b') (Suc (Suc (Suc n))) = 0 \<and> b' 1 = a 1 \<and> b' (Suc (Suc n)) = b0 (Suc (Suc n))"
        using pre_hensel A0 assms 
        by auto 
      have A2: "b1 = (SOME b. b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc (Suc (Suc n))) = 0 \<and> b 1 = a 1 \<and> b (Suc (Suc n)) = b0 (Suc (Suc n)))"
        using A 
        by simp
      let ?P = "(\<lambda> b. b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc (Suc (Suc n))) = 0 \<and> b 1 = a 1 \<and> b (Suc (Suc n)) = b0 (Suc (Suc n)))"
      show ?thesis 
...
lemma hensel_seq_id_0:
  ...
  shows "b \<in> carrier Z\<^sub>p \<and> (f \<^emph> b) (Suc n) = 0 \<and> b 1 = a 1" 
proof(cases "n = 0")
  case True
  then have "a = b" 
    using assms by simp
...
lemma hensels_lemma:
assumes "hensel f a"
shows "\<exists> \<alpha> \<in> carrier Z\<^sub>p. f\<^emph>\<alpha> = \<zero> \<and> \<alpha> 1 = a 1"
proof-
  ...
  have Cont: "is_continuous (fun_of f)"
    ...
    by blast
  have A0: "(f\<^emph>\<alpha>) = res_lim ((fun_of f)\<^sub>*s)"
