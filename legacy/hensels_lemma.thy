...
imports "~~/src/HOL/Algebra/UnivPoly" padic_int_topology domain_poly padic_fields
begin
...
"hensel f a \<equiv> (f \<in> carrier Z\<^sub>p_x \<and> a \<in> carrier Z\<^sub>p \<and> ((f \<^emph> a) 1) = 0 \<and> (deriv f a 1) \<noteq> 0)"
...
proof-
    ...
    next
      ...
      then have "f \<^emph> b \<in>nonzero Z\<^sub>p"
        using assms(1) b_def not_nonzero_Zp 
        by (meson Z\<^sub>p_x_is_UP_domain fun_of_closed_UP_domain)
      then have "ord_Zp (f \<^emph> b) \<ge> Suc n" 
        using b_def  False Zp_nonzero_def(1) ord_Zp_geq by metis
      then have "ord_Zp (\<p>[^](Suc n)) \<le> ord_Zp (f \<^emph> b)" 
        using b_def  ord_Zp_p_pow 
        by presburger
      then show ?thesis 
        using False G_ord(2) \<open>int (Suc n) \<le> ord_Zp (f \<^emph> b)\<close> 
          ord_Zp_def val_Zp_def val_Zp_p_pow[of "Suc n"] 
        by (metis val_Zp_p_pow )
    qed
    then have 1:"divide (f \<^emph> b) (\<p>[^](Suc n)) \<otimes> (\<p>[^](Suc n)) = f \<^emph> b"
      using assms b_def continuous_is_closed divide_id'(2)  is_closed_fun_simp
        p_pow_nonzero polynomial_is_continuous 
      by presburger
    ...
    proof-
      ...
      have P1: "(deriv f a) 1 \<in> carrier (R 1)"
        using 2  Z\<^sub>p_def by (metis padic_set_simp0 partial_object.select_convs(1))
      ...
      then have P2: "k \<otimes>\<^bsub>R 1\<^esub>((deriv f a) 1) = \<one>\<^bsub>R 1\<^esub>"
        using residue_ring_def  P0 P1  field.field_inv[of "R 1" "(deriv f a) 1"] k_def
        by metis 
    ...
    proof-
      ...
      have c_car:"c \<in> carrier Z\<^sub>p"
        using Z\<^sub>p_x_is_UP_domain Taylor_closed_UP_domain 
      ...
      proof-
        ...
        have P1: "f \<in> carrier Z\<^sub>p_x"
          using assms by auto 
        have P2: "b \<in> carrier Z\<^sub>p"
          using b_def by auto 
        show ?thesis
          using P0 P1 P2   fb_def f'b_def c_def  
                Z\<^sub>p_x_is_UP_domain UP_domain.Taylor_deg_1_eval[of Z\<^sub>p f b "(\<p>[^](Suc n)) \<otimes> t" c fb f'b]
        by auto 
      qed
      ...
      proof-
        ...
        next
          ...
          have Q2: "ord_Zp ((\<p> [^] (Suc n)) [^] (2::nat))  = 2*(Suc n)"
            using  ord_Zp_p_pow ord_Zp_pow p_pow_nonzero by presburger
      ...
      proof-
        have A20: "f'b \<in> carrier Z\<^sub>p"
          using f'b_def  assms b_def UP_domain.derivative_closed[of Z\<^sub>p f b] Z\<^sub>p_x_is_UP_domain  
          by blast
      ...
      have A4: " (f \<^emph> (\<p> [^] (Suc n) \<otimes> t \<oplus> b)) (Suc (Suc n)) 
        ...
        by (metis p_pow_rep0 scalar_mult_def zero_less_Suc) 
      ...
      proof-
        ...
        proof-
          ...
          proof-
            ...
            have C11:"(f'a \<otimes> t) 1  = (f'a 1) \<otimes>\<^bsub>R 1\<^esub>(\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<otimes>\<^bsub>R 1\<^esub> k))"
              using C10 m_def C0 
              by (smt R_residues  mod_mult_right_eq of_nat_0_less_iff 
            ...
            proof-
              have C120:"((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<in> carrier (R 1)"
                by (metis A7  Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> 
              ...
              have C122: "(\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1) \<otimes>\<^bsub>R 1\<^esub> k))
                ...
                by (metis R_residues  mult.commute nat_less_le 
                    residues.res_mult_eq zero_le_one zero_neq_one)
              have C123: "f'a 1 \<in> carrier (R 1)"
                by (metis "2"  Z\<^sub>p_def f'a_def padic_set_simp0 partial_object.select_convs(1))
              show ?thesis
                unfolding a_minus_def
            ...
            proof-
              ...
              then have "(divide (f \<^emph> b) (\<p>[^](Suc n))) 1 \<in> carrier (R 1)"
                by (metis  Z\<^sub>p_def padic_set_simp0 partial_object.select_convs(1))
              then show ?thesis using Res_1_field C13 
                by (metis le_eq_less_or_eq not_one_le_zero res_mult_closed zero_le_one) 
            qed
            ...
            then have "(f'a \<otimes> t) 1  = (\<ominus>\<^bsub>R 1\<^esub> ((f'a 1) \<otimes>\<^bsub>R 1\<^esub> k ) \<otimes>\<^bsub>R 1\<^esub>(((divide (f \<^emph> b) (\<p>[^](Suc n))) 1)))"
              using C12 C13 C14 C15 Res_1_field R_cring[of 1] R_residues[of 1]
              by (metis (no_types, lifting) A7 Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> 
                  cring.cring_simprules(11) cring.cring_simprules(28) cring.cring_simprules(3)
                  divide_id'(1) fb_def p_pow_nonzero padic_set_simp0 partial_object.select_convs(1)
                  zero_less_one)
            then have  "(f'a \<otimes> t) 1  = (\<ominus>\<^bsub>R 1\<^esub> (((divide (f \<^emph> b) (\<p>[^](Suc n))) 1)))"
              ...
              by (smt A7  Z\<^sub>p_def \<open>k \<otimes>\<^bsub>R 1\<^esub> deriv f a 1 = \<one>\<^bsub>R 1\<^esub> \<and> k \<in> carrier R 1\<close>
                  \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> cring.cring_simprules(12) 
                  cring.cring_simprules(14) cring.cring_simprules(29) cring.cring_simprules(3)  
                  cring.cring_simprules(5) divide_id'(1) f'a_def fb_def p_pow_nonzero 
        ...
        then have A81: "(f'a \<otimes> t) 1= (\<ominus> (divide (fb) (\<p>[^](Suc n)))) 1"
          using res_uminus A7  Z\<^sub>p_def \<open>val_Zp (f \<^emph> b) \<succeq>\<^bsub>G\<^esub> val_Zp (\<p> [^] Suc n)\<close> 
            divide_id'(1) p_pow_nonzero padic_inv prime 
          by (metis (no_types, lifting) fb_def)
        ...
        proof- 
          ...
          have A832: "f'b \<in> carrier Z\<^sub>p"
            using f'b_def b_def assms UP_domain.derivative_closed[of Z\<^sub>p f b] Z\<^sub>p_x_is_UP_domain 
            by auto 
...
proof-
  ...
  have A0: "(f\<^emph>\<alpha>) = res_lim ((fun_of f)\<^sub>*s)"
    using C Cont alpha_def alt_seq_limit
          res_lim_pushforward res_lim_pushforward'
