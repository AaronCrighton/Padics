...
(*************************************************************************************************)
(*************************************************************************************************)
(***************************** NOTATION AND FACTS ABOUT VALUE GROUP ******************************)
(*************************************************************************************************)
(*************************************************************************************************) 
...
lemma(in padic_integers) G_mult:
...
proof-
  have "G = int_eoag" 
    by (simp add: G_def) 
  show "\<infinity>\<^bsub>G\<^esub> \<otimes>\<^bsub>G\<^esub> x  = \<infinity>\<^bsub>G\<^esub>" 
  proof(cases "x = None")
    case True
    then show ?thesis 
      by (simp add: G_def) 
  next
    ...
    then show ?thesis 
      using G_def by auto 
    qed
  show "x \<otimes>\<^bsub>G\<^esub> \<infinity>\<^bsub>G\<^esub> = \<infinity>\<^bsub>G\<^esub>"
  proof(cases "x = None")
    case True
    then show ?thesis 
      by (simp add: G_def) 
  next
    ...
    then show ?thesis 
      using G_def by auto 
    qed
  show "Some a \<otimes>\<^bsub>G\<^esub> Some b = Some (a + b)"  
  proof-
    have "Some a \<otimes>\<^bsub>G\<^esub> Some b =  Some (a \<otimes>\<^bsub>int_oag\<^esub> b) "
      by (simp add: G_def)
    then show ?thesis 
      by simp 
  qed
...
"neg (Some n) = *-n*"
  using G_def G_is_group G_mult(3) add.left_inverse group.inv_equality by fastforce

lemma(in padic_integers) G_ord:
"x \<preceq>\<^bsub>G\<^esub> \<infinity>\<^bsub>G\<^esub>"
"((Some a) \<preceq>\<^bsub>G\<^esub> (Some b)) \<Longrightarrow> (a \<le> b)"
"(a \<le> b) \<Longrightarrow> ((Some a) \<preceq>\<^bsub>G\<^esub> (Some b))"
proof-
  show "x \<preceq>\<^bsub>G\<^esub> \<infinity>\<^bsub>G\<^esub>"
  proof(cases "x = None")
    case True
    then show ?thesis 
      by (simp add: G_def) 
  next
    ...
    then show ?thesis 
      using G_def by auto 
  qed
  show "((Some a) \<preceq>\<^bsub>G\<^esub> (Some b)) \<Longrightarrow> (a \<le> b)"
    by (simp add: G_def) 
  show "(a \<le> b) \<Longrightarrow> ((Some a) \<preceq>\<^bsub>G\<^esub> (Some b))"
    by (simp add: G_def) 
qed

lemma(in padic_integers) Min_min:
"Min\<^bsub>G\<^esub> (Some n) (Some m) = *(min n m)*"
  by (metis G_ord(2) G_ord(3) le_cases min.absorb2 min.commute)
...
definition(in padic_integers) Res:: "nat \<Rightarrow> _ ring" ("R _")where
"(Res n) \<equiv> residue_ring (int (p^n))"

lemma(in padic_integers) r_range':
"r m n \<in> carrier (R m)"
  using r_range Res_def residue_ring_def prime prime_gt_0_nat r_def by fastforce
...
proof-
  ...
  then show ?thesis using Res_def residues_prime.is_field 
    by (simp ) 
qed
...
lemma(in padic_integers) R_cring:
  assumes "m >0"
  shows "cring (R m)"
  using R_residues Res_def assms residues.cring by auto 
...
lemma(in padic_integers) Qp_is_domain:
"domain Q\<^sub>p"
  using Qp_is_field field_def by auto
...
"\<one> \<in> carrier Z\<^sub>p" 
  by (simp add: Zp_is_domain cring.cring_simprules(6) domain.axioms(1))

lemma(in padic_integers) Zp_one_notzero:
"\<one> \<noteq>\<zero>"
  by (simp add: Zp_is_domain domain.one_not_zero)
...
"\<one>\<^bsub>Q\<^sub>p\<^esub> \<in> carrier Q\<^sub>p" 
  by (simp add: Qp_is_field cring.cring_simprules(6) fieldE(1))

lemma(in padic_integers) Qp_one_notzero:
"\<one>\<^bsub>Q\<^sub>p\<^esub> \<noteq>\<zero>\<^bsub>Q\<^sub>p\<^esub>"
  by (simp add: Qp_is_field one_not_zero)
...
"\<iota> \<one> = \<one>\<^bsub>Q\<^sub>p\<^esub>"
  by (simp add: Localization.submonoid.one_closed Q\<^sub>p_def 
      Zp_is_domain cring.cring_simprules(6) domain.axioms(1)
...
lemma(in padic_integers) inc_of_sum:
  ...
  by (simp add: Localization.submonoid.one_closed Q\<^sub>p_def Zp_is_domain assms(1)
...
lemma(in padic_integers) inc_of_prod:
  ...
  using Localization.submonoid.one_closed Q\<^sub>p_def Zp_is_domain 
    ...
    local.frac_def local.inc_def by fastforce
...
next
  ...
  have  "\<iota> a [^]\<^bsub>Q\<^sub>p\<^esub> (Suc n) = (\<iota> a) \<otimes>\<^bsub>Q\<^sub>p\<^esub> (\<iota> a)[^]\<^bsub>Q\<^sub>p\<^esub>n" 
    by (simp add: iA QD domain.pow_suc) 
...
lemma(in padic_integers) Units_nonzero_Zp:
assumes "u \<in> Units Z\<^sub>p"
shows "u \<in> nonzero Z\<^sub>p"
  by (simp add: Zp_is_domain assms domain.Units_nonzero)

lemma(in padic_integers) Units_nonzero_Qp:
assumes "u \<in> Units Q\<^sub>p"
shows "u \<in> nonzero Q\<^sub>p"
  by (simp add: Qp_is_field assms domain.Units_nonzero field.axioms(1))
...
lemma(in padic_integers) frac_add:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) assms(3) assms(4) domain.frac_add local.frac_def)
...
lemma(in padic_integers) frac_add_common_denom:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) assms(3) domain.frac_add_common_denom local.frac_def)
...
lemma(in padic_integers) frac_mult:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) assms(3) assms(4) domain.frac_mult local.frac_def)
...
lemma(in padic_integers)  frac_closed:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.frac_im local.frac_def)
...
lemma(in padic_integers) nonzero_numer_imp_nonzero_fraction:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.nonzero_fraction local.frac_def)
...
lemma(in  padic_integers) frac_inv:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.frac_inv local.frac_def)
...
lemma(in  padic_integers) frac_uminus:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.frac_uminus local.frac_def)
...
lemma(in  padic_integers) frac_cancel_l:
  ...
  by (simp add: Zp_is_domain assms(1) assms(2) assms(3) domain.frac_cancel_l local.frac_def)
...
lemma(in  padic_integers) frac_cancel_r:
  ...
  by (simp add: Zp_is_domain assms(1) assms(2) assms(3) domain.frac_cancel_r local.frac_def)
...
lemma(in  padic_integers) frac_cancel_lr:
  ...
  by (simp add: Zp_is_domain assms(1) assms(2) assms(3) domain.frac_cancel_lr local.frac_def)
...
lemma(in  padic_integers) frac_cancel_rl:
  ...
  by (simp add: Zp_is_domain assms(1) assms(2) assms(3) domain.frac_cancel_rl local.frac_def)
...
lemma(in  padic_integers) i_mult:
  ...
  by (simp add: Q\<^sub>p_def Zp_is_domain \<iota>_def assms(1) assms(2) assms(3) domain.i_mult local.frac_def)
...
lemma(in padic_integers) numer_denom_facts:
  ...
  using Q\<^sub>p_def Zp_is_domain assms domain.numer_denom_facts(1) numer_def apply auto[1]
  using Q\<^sub>p_def Zp_is_domain assms denom_def domain.numer_denom_facts(2) apply auto[1]
  using Q\<^sub>p_def Zp_is_domain assms domain.numer_denom_facts(3) numer_def apply auto[1]
  using Q\<^sub>p_def Zp_is_domain \<iota>_def assms denom_def domain.numer_denom_facts(4) numer_def apply auto[1]
  using Q\<^sub>p_def Z\<^sub>p_def assms denom_def domain.numer_denom_facts(5) local.frac_def numer_def
    padic_integers.Zp_is_domain padic_integers_axioms by auto
...
lemma(in padic_integers) get_common_denominator:
  ...
  obtains a b c where
    ...
    "x = frac a c"
    "y = frac b c"
  using Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.common_denominator local.frac_def that by auto
...
proof-
  ...
  have P0:"(inv\<^bsub>Q\<^sub>p\<^esub> (\<iota> b)) = frac \<one> b" 
    using B inc_def frac_inv by (simp add: Localization.submonoid.one_closed  
        Zp_is_domain assms(2) domain.nonzero_is_submonoid)
...
proof(induction n)
  case 0
  then show ?case 
    using Zp_is_domain cring.cring_simprules(2) domain_def Zp_nat_inc_zero by auto 
next
  case (Suc n)
  then show ?case 
...
proof(induction n)
  case 0
  then show ?case 
    using Zp_is_domain cring.cring_simprules(2) domain_def Zp_nat_mult_zero by auto 
next
  case (Suc n)
  then show ?case 
    using Zp_is_domain domain_def 
  ...
  proof fix m
    ...
    proof(cases "m=0")
      ...
      have 0: "([Suc n] \<cdot> \<one>) m \<in> carrier (R m)" 
        using Zp_nat_inc_closed padic_set_simp0 by (simp add: Res_def Z\<^sub>p_def)
      ...
      next
        ...
        then have P0: "([Suc n] \<cdot> \<one>) m  =  r m (int n) \<oplus>\<^bsub>R m\<^esub> \<one>\<^bsub>R m\<^esub>" 
          using A False Z\<^sub>p_def padic_add_simp padic_one_def 
            \<open>residues (int (p ^ m))\<close> residues.res_one_eq Res_def by auto 
        then have P1:"([Suc n] \<cdot> \<one>) m =  r m (int n) \<oplus>\<^bsub>R m\<^esub> r m (1::int)"
          using \<open>residues (int (p ^ m))\<close> res_def residues.one_cong Res_def by auto
        have P2: "r m (int n) \<oplus>\<^bsub>R m\<^esub> r m 1 = ((int n) mod (p^m)) \<oplus>\<^bsub>R m\<^esub> 1" 
          using R P0 P1 res_def residues.res_one_eq Res_def by auto 
        have P3:"((int n) mod (p^m)) \<oplus>\<^bsub>R m\<^esub> 1 = ((int n) + 1) mod (p^m)" 
          using R residue_ring_def  by (simp add: Res_def mod_add_left_eq) 
...
next
  ...
  proof-
    ...
    then have L1: "([Suc m] \<cdot> ([n] \<cdot> \<one>)) k = (([m] \<cdot> ([n] \<cdot> \<one>)) k) \<oplus>\<^bsub>R k\<^esub> ( ([n] \<cdot> \<one>) k) "
      using Z\<^sub>p_def padic_add_def by (simp add: Res_def) 
    ...
    then have L4: "([Suc m] \<cdot> ([n] \<cdot> \<one>)) k = (( r k (m*n)) + (r k n)) mod (p^k)"
      using residue_ring_def Res_def by auto 
      ...
      have  "\<ominus>\<^bsub>R m\<^esub>(r m (Suc n)) =  r m (- int (Suc n))" 
        proof(cases "m=0")
          ...
          then have 3:"\<ominus>\<^bsub>R m\<^esub>(r m (Suc n)) =0" 
            using res_1_prop 
            by (metis Res_def One_nat_def Totient.of_nat_eq_1_iff True 
        ...
        next
          ...
          have "\<ominus>\<^bsub>R m\<^esub> r m (int (Suc n)) = \<ominus>\<^bsub>R m\<^esub> (int (Suc n)) mod (p^m) " 
            using R res_def residues.neg_cong residues.res_neg_eq Res_def by auto 
          then have "\<ominus>\<^bsub>R m\<^esub> r m (int (Suc n)) = (-(int (Suc n))) mod (p^m)" 
            using R residues.res_neg_eq Res_def by auto 
          then show ?thesis 
            by (simp add: res_def) 
        qed
      then show ?thesis  
...
proof-
  ...
  then have 0: "\<p> 1 = \<zero>\<^bsub>R 1\<^esub>"  
    using padic_zero_def padic_zero_simp Res_def by presburger  
  ...
  then have 4:"ord_Zp \<p> \<le> 1"     
    ...
    by (metis Res_def One_nat_def Z\<^sub>p_def int_ops(2) numeral_2_eq_2
...
proof-
  have "ord_Zp \<one> = ord_Zp (\<one> \<otimes> \<one>)" 
    by (simp add: Zp_is_domain cring.cring_simprules(12) cring.cring_simprules(6) domain.axioms(1)) 
...
proof(induction n)
  case 0
  have "x[^](0::nat) = \<one>" 
    using assms(1) nonzero_def by (simp add: Zp_is_domain domain.pow_0)
  then show ?case 
...
next
  ...
  proof-
    have "ord_Zp x \<ge> 0"
      using assms by (simp add: Zp_nonzero_def(1) Zp_nonzero_def(2) ord_pos)
  ...
  have "x[^](Suc n) = x \<otimes>(x[^]n)" 
    by (simp add: Zp_is_domain Zp_nonzero_def(1) assms domain.pow_suc)
...
proof-
  ...
  have "(x \<otimes>\<^bsub>Z\<^sub>p\<^esub> y) \<noteq> \<zero>"
    by (simp add: Zp_is_domain assms(1) assms(2) assms(3) assms(4) domain.integral_iff) 
...
proof(cases "x = \<zero> \<or> y = \<zero>")
  ...
  then have T0: "(x \<otimes>\<^bsub>Z\<^sub>p\<^esub> y) = \<zero>" 
    by (simp add: Zp_is_domain assms(1) assms(2) domain.integral_iff) 
...
using assms  by (simp add: nonzero_def) 
  have "x \<oplus>y \<in>carrier Z\<^sub>p" using assms 
    by (simp add: Zp_is_domain cring.cring_simprules(1) domain.axioms(1)) 
  ...
  have 1: "Min\<^bsub>G\<^esub> (val_Zp x) (val_Zp y) = Some (min (ord_Zp x) (ord_Zp y))"
    by (simp add: G_ord(2) G_ord(3) assms(2) assms(4) min_def ord_Zp_def val_Zp_def) 
  have 2: "(min (ord_Zp x) (ord_Zp y)) \<le> (ord_Zp (x \<oplus> y))"
    using Nx Ny Nxy ord_Zp_ultrametric by blast  
  show ?thesis using G_ord 0 1 by (simp add: "2")  
qed
...
proof(cases "x = \<zero> \<or> y = \<zero>")
  ...
  proof(cases "x = \<zero>")
    ...
    have T1: "val_Zp (x \<oplus> y) = val_Zp y"
      using True by (simp add: Zp_is_domain assms(2) cring.cring_simprules(8) domain.axioms(1)) 
  ...
  next
    ...
    then have F0:" Min\<^bsub>G\<^esub> (val_Zp x) (val_Zp y) = (val_Zp x)" 
      using G_ord(1) True val_Zp_def by auto 
    have F1: "val_Zp (x \<oplus> y) = val_Zp x"
      using False True Zp_is_domain assms(1) cring.cring_simprules(16) domain.axioms(1) by fastforce 
    then show ?thesis 
      using F0 F1 by (simp add: False G_ord(3) val_Zp_def) 
  qed
next
  case False 
  then show ?thesis 
    using val_Zp_ultrametric0 assms G_ord(1) val_Zp_def by auto 
qed
...
proof-
  ...
  then have "coprime (x (Suc n)) (p^(Suc n))"
    by blast 
  then show ?thesis using residues.res_units_eq  Res Res_def 
...
proof-
    ...
    proof-
      ...
      next
        ...
        have I:"?y m = m_inv (residue_ring (p^m)) (x m)"
          using False  by (simp add: Res_def) 
        show ?thesis using residues.cring U I Res 
          by (metis False Res_def \<open>x m \<in> Units R m\<close> cring_def
  ...
  proof
    ...
    proof(cases "m=0")
      ...
      have R: "\<one> m = 0" 
        by (simp add: True Zp_is_domain cring.cring_simprules(6) domain.axioms(1) ord_Zp_one zero_below_ord) 
      ...
      next
        ...
        proof-
          ...
          have U: "(x m) \<in> Units (residue_ring (p^m))" 
            by (metis (mono_tags, hide_lams) False Res_def 
  ...
  have "\<exists>y\<in>carrier Z\<^sub>p. x \<otimes>y = \<one> \<and> y \<otimes> x = \<one>" 
    using 1 2 3 Zp_is_domain assms(2) cring.cring_simprules(14) domain.axioms(1) by force
  then show ?thesis 
...
proof-
  ...
  then have "(ord_Zp a) + (ord_Zp  d) = (ord_Zp b) + (ord_Zp c)"
    using assms(1) assms(2) assms(3) assms(4) ord_Zp_mult by auto 
  then show ?thesis 
    by linarith 
qed
...
proof- 
  ...
  then show ?thesis 
    by (simp add: "0" assms(1) assms(2) assms(3) assms(4) assms(5) ord_Zp_eq_frac)
qed
...
proof(induction n)
  ...
  proof
    assume "\<p> = \<zero>"
    then have 0: "ord_Zp \<p> = ord_Zp \<zero>" by auto
    have 1: "ord_Zp \<p> = 1" 
      by (simp add: ord_Zp_p) 
    have 2: "ord_Zp \<zero> = -1" 
      using ord_Zp_def padic_val_def Z\<^sub>p_def by simp
    then show False 
      using "0" ord_Zp_p by auto 
  qed
  ...
  proof
    ...
    next
      ...
      then have "(\<p> [^] Suc n) x = ([p]\<cdot>\<one>) x  \<otimes>\<^bsub>R x\<^esub> ([(p ^ n)] \<cdot> \<one>) x" 
        using Z\<^sub>p_def Res_def by (simp add: padic_mult_def) 
      then have "(\<p> [^] Suc n) x =  r x p  \<otimes>\<^bsub>R x\<^esub> r x (p ^ n)" 
        using Zp_nat_inc_rep  by simp 
      then have "(\<p> [^] Suc n) x =  ( p  * (p ^ n)) mod (p^x)" 
        using Res_def r_def residue_ring_def  by (simp add: mod_mult_eq of_nat_mod) 
...
proof-
  have X0: "(x (m+k)) \<in> carrier (R (m+k)) " 
    using Z\<^sub>p_def assms(3) padic_integers.Res_def padic_integers_axioms padic_set_simp0 by auto
 ...
 then obtain u where U0: " u*(int p^m) = (x (m+k))" 
  ...
  have X3: "(x (m+k)) < p^(m+k)" 
    using assms(3) X0 Res_def R_residues assms(2) residues.res_carrier_eq by auto
  have U3: "u < p^k" 
  proof(rule ccontr)
    assume "\<not> u < (p ^ k)"
    then have "(p^k) \<le> u" 
  ...
  then have "u \<in> carrier (R k)" 
    using assms(2) Res_def R_residues residues.res_carrier_eq U3 U2 by auto
...
definition(in padic_integers) ac where 
"ac x = normalizer (nat (ord_Zp x)) x"


lemma(in padic_integers) ac_equation:
  assumes "x \<in> carrier Z\<^sub>p"
  assumes "x \<noteq>\<zero>"
  assumes "k > 0"
  assumes "m = nat (ord_Zp x)"
  shows "(ac x k) \<in> carrier (R k) \<and> (ac x k)*(p^m) = (x (m+k))" 
proof-
  have K0: "k >0" 
  ...
  have RHS: "ac x k = (THE u. u \<in> carrier (R k) \<and> u*(p^m) = (x (m+k)))" 
  proof-
    have K: "k \<noteq>0" 
      by (simp add: K0)
    have "ac x k = normalizer (nat (ord_Zp x)) x k" 
      by (simp add: ac_def)
    then have "ac x k = normalizer m x k" 
      using assms by blast
    then show "ac x k = (THE u. u \<in> carrier (R k) \<and> (u* p^m) = (x (m + k)) ) "
      using normalizer_def K by simp   
  qed
  have LHS:"u = (THE u. u \<in> carrier (R k) \<and> u*(p^m) = (x (m+k)))" 
    using assms U0 K0 assms(1) res_factor_unique  by auto
  then have "u = ac x k" 
    by (simp add: RHS)
...
lemma(in padic_integers) ac_res[simp]:
  ...
  assumes "x \<in> carrier Z\<^sub>p"
  assumes "x \<noteq>\<zero>"
  shows "r k (ac x m) = (ac x k)"
proof(cases "k =0")
  case True
  then show ?thesis 
    by (metis ac_def mod_by_1 normalizer_def of_nat_1 power.simps(1) r_def)
next
  case False
  then have K0: "k >0" by simp
  obtain uk where Uk0: "uk = (ac x k)" 
    by simp
  obtain um where Um0: "um = (ac x m)" 
    by simp
  have Uk1: "uk \<in> carrier (R k) \<and> uk*(p^n) = (x (n+k))" 
    using K0 Uk0 ac_equation assms(2) assms(3) assms(4) by auto
  have Um1: "um \<in> carrier (R m) \<and> um*(p^n) = (x (n+m))" 
    using Uk1 Um0 ac_equation assms(1) assms(3) assms(4) assms(2) by auto
  have "um mod (p^k) = uk"
  proof-
    ...
    then show ?thesis  using Uk1 Res_def 
...
proof-
  have "ac x \<in> padic_set p"
  proof(rule padic_set_mem)
    show "\<And>m. ac x m \<in> carrier (residue_ring (int p ^ m))"
    proof- 
      fix m
      show "ac x m \<in> carrier (residue_ring (int p ^ m))"
      proof(cases "m = 0")
        case True
        then have "ac x m = 0" 
          using ac_def normalizer_def by simp
        then show ?thesis 
      ...
      next
        ...
        then have "m>0" 
          by blast
        then show ?thesis 
          using ac_equation by (simp add: Res_def assms(1) assms(2))
      qed
    qed
    show "\<And>m n. m < n \<Longrightarrow> res (int p ^ m) (ac x n) = ac x m"
      using ac_res assms(1) assms(2) by auto
  qed
  show ?thesis 
...
lemma(in padic_integers) ac_factors_x:
  assumes "x \<in> carrier Z\<^sub>p"
  assumes "x \<noteq>\<zero>"
  shows "x = (\<p>[^](nat (ord_Zp x))) \<otimes> (ac x)"
proof
  fix k
  show "x k = ((\<p>[^](nat (ord_Zp x))) \<otimes> (ac x)) k"
  proof(cases "k=0")
    case True
    then show ?thesis 
      by (metis Z\<^sub>p_def Zp_is_domain ac_in_Zp assms(1) assms(2) cring.cring_simprules(5)
  ...
  next
    ...
    next
      ...
      obtain m where M0: "k = n + m" 
        using False N le_Suc_ex by fastforce
      have M1: "m >0" 
        using M0 False N assms(1) assms(2) ord_nat by auto
      have E1: "(ac x m)*(p^n) = (x k)" 
        using M0 M1 N ac_equation assms(1) assms(2) by blast
      have E2: "(ac x k)*(p^n) = (x (n + k))" 
        using M0 M1 N ac_equation assms(1) assms(2) by auto
      have E3: "((ac x k) mod (p^k))*((p^n) mod p^k) mod (p^k) = (x (n + k)) mod (p^k)"
        by (metis E2 mod_mult_left_eq mod_mult_right_eq of_nat_mod)
      have E4: "((ac x k) mod (p^k))*(p^n) mod (p^k) = (x k)" 
        by (metis E2 assms(1) le_add2 mod_mult_left_eq r_Zp r_def)
      have E5: "(ac x k)*((p^n) mod p^k) mod (p^k) = (x k)" 
        using E2 assms(1) r_Zp r_def by (metis E3 E4 mod_mult_left_eq)
      have E6: "(ac x k) \<otimes>\<^bsub>(R  k)\<^esub> ((p^n) mod p^k)  = (x k)" 
        using E5 M0 M1 R_residues Res_def residues.res_mult_eq by auto
      have E7: " ((p^n) mod p^k) \<otimes>\<^bsub>(R  k)\<^esub>(ac x k)   = (x k)" 
        by (metis E5 M0 M1 R_residues Res_def add_gr_0 mult_of_nat_commute residues.res_mult_eq)
      have E8: "((\<p>[^](nat (ord_Zp x))) k) \<otimes>\<^bsub>(R  k)\<^esub> (ac x k) = (x k)" 
        using E7 N p_pow_rep by auto
      then show ?thesis 
...
"subring \<O>\<^sub>p Q\<^sub>p"
  by (simp add: Q\<^sub>p_def Zp_is_domain \<iota>_def domain.inc_im_is_subring) 
...
proof-
  ...
  have P5: "\<one> \<in> nonzero Z\<^sub>p" 
    using nonzero_def by (simp add: Localization.submonoid.one_closed
        Zp_is_domain domain.nonzero_is_submonoid) 
  have "(frac (\<p>[^](m-n)) \<one>) \<otimes>\<^bsub>Q\<^sub>p\<^esub> (frac (\<p>[^]n) (\<p>[^]n)) 
        = frac ((\<p>[^](m-n))\<otimes> (\<p>[^]n)) ((\<one> \<otimes> (\<p>[^]n)))"
    using Zp_is_domain  P2 P4 P3 P5 frac_def Q\<^sub>p_def by (simp add: domain.frac_mult) 
  then have "frac ((\<p>[^](m-n))\<otimes> (\<p>[^]n)) ((\<one> \<otimes> (\<p>[^]n))) = (frac (\<p>[^](m-n)) \<one>) "
    by (simp add: domain.frac_cancel_r P2 P4 P5 Zp_is_domain local.frac_def)  
...
proof-
  have "\<pp> = frac \<p> \<one>" 
    by (simp add: domain.nat_inc_rep Q\<^sub>p_def Zp_is_domain local.frac_def)
  then show ?thesis 
...
lemma(in padic_integers) p_natpow_inc:
  ...
  by (simp add: inc_pow Zp_nat_inc_closed ord_Zp_p ord_of_nonzero(2) p_inc)

lemma(in padic_integers) p_intpow_inc:
  fixes n::int
  assumes "n \<ge>0"
  shows "\<pp> e n = \<iota> (\<p> [^] n)"
  using p_natpow_inc  by (metis assms int_nat_eq int_pow_int)
...
proof-
  have "frac a b = frac (numer (frac a b)) (denom (frac a b))"
    using Q\<^sub>p_def Zp_is_domain assms(1) assms(2) domain.frac_im 
      local.frac_def Zp_nonzero_def(1) numer_denom_facts(5) by force
  then have "a \<otimes> (denom (frac a b)) = b \<otimes> (numer (frac a b))"
    by (simp add: Zp_is_domain assms(1) assms(2) denom_def domain.frac_eq domain.frac_im 
        domain.numer_denom_facts(1) domain.numer_denom_facts(2) local.frac_def 
        Zp_nonzero_def(1) numer_def)
...
proof(cases "a = \<zero>")
  ...
  then show ?thesis 
     ...
     using nonzero_fraction_imp_numer_not_zero by fastforce
next
  case False
...
proof-  
  have 0: "frac v v = \<one>\<^bsub>Q\<^sub>p\<^esub>" using frac_one
    by (simp add: Units_nonzero_Zp assms(2))
  ...
  then have 4:  "frac (u \<otimes> (inv\<^bsub>Z\<^sub>p\<^esub> v))  \<one> =  (frac (u \<otimes> ((inv\<^bsub>Z\<^sub>p\<^esub> v) \<otimes> v))  v)"
    by (simp add: "1" Units_nonzero_Zp Zp_is_domain assms(1)
        assms(2) cring.cring_simprules(11) domain.axioms(1) Zp_nonzero_def(1))
...
lemma(in padic_integers) Zp_division_Qp_1:
  ...
  obtains w where "w \<in> Units Z\<^sub>p"
                  "\<iota> w = frac u v"
proof-
  have " (inv\<^bsub>Z\<^sub>p\<^esub> v) \<in> Units Z\<^sub>p" using Zp_is_domain domain_def cring_def ring_def assms(2) 
    by (simp add: cring_def domain_def ring_def monoid.Units_inv_Units)
  then have "(u \<otimes> (inv\<^bsub>Z\<^sub>p\<^esub> v)) \<in> Units Z\<^sub>p" 
    using assms Zp_is_domain domain_def cring_def ring_def  
    by (simp add: cring_def domain_def ring_def monoid.Units_m_closed)
...
proof-
  ...
  proof-
    have "ord_Zp c \<ge> ord_Zp d" 
      using P0 P1 P2 assms(3) ord_of_frac by auto
    then show ?thesis 
      by (simp add: P3 P4)
  qed
  have P16: "n \<ge> 0" 
  ...
  have P24:  "a = \<iota>((\<p>[^](m-n)) \<otimes> w)" 
    by (simp add: P20 P23 P24 Zp_is_domain cring.cring_simprules(5) domain.axioms(1) local.inc_def)
  then show ?thesis 
...
proof-
  obtain b c where 
   ...
   Nz_c: "c \<in> nonzero Z\<^sub>p"
      ...
      local.frac_def by fastforce
    have Nz_b: "b \<in> nonzero Z\<^sub>p" 
      using Frac Car Nz_c  assms(2) nonzero_fraction_imp_nonzero_numer by auto
    then have "(inv\<^bsub>Q\<^sub>p\<^esub> a) = frac c b" 
      by (simp add: Frac Nz_c frac_inv)
    then show ?thesis using Frac ord_of_frac 
      by (simp add: Nz_b Nz_c)
  qed
...
proof-
  ...
  have An: "a \<in> nonzero Z\<^sub>p" 
    using A C Fx assms(1) Qp_nonzero_def(2) nonzero_fraction_imp_nonzero_numer by auto
  have Bn: " b \<in> nonzero Z\<^sub>p" 
    using B C Fy assms(2)  Qp_nonzero_def(2) nonzero_fraction_imp_nonzero_numer by auto
  ...
  proof-
    ...
    then show ?thesis 
      using Fxy C assms(3) nonzero_def Qp_nonzero_def(2)
        nonzero_fraction_imp_nonzero_numer by auto
  qed
  have Ordx: "ord x = ord_Zp a - ord_Zp c"
      using Fx An C ord_of_frac by auto  
  have Ordy: "ord y = ord_Zp b - ord_Zp c" 
    using Fy Bn C ord_of_frac by auto  
...
proof-
  have 0: "val (x \<oplus>\<^bsub>Q\<^sub>p\<^esub> y) = *ord (x \<oplus>\<^bsub>Q\<^sub>p\<^esub> y)*" 
    using assms(3) nonzero_def val_def by fastforce
  have 1: "val x = *ord x*" 
...
proof(cases "x = \<zero>\<^bsub>Q\<^sub>p\<^esub> \<or> y = \<zero>\<^bsub>Q\<^sub>p\<^esub>")
  ...
  proof(cases "x = \<zero>\<^bsub>Q\<^sub>p\<^esub>")
    ...
    have T1: "val (x \<oplus>\<^bsub>Q\<^sub>p\<^esub> y) = val y"
      ...
      by (simp add: domain_def field_def assms(2) cring.cring_simprules(8))
    then show ?thesis 
      using T0 T1 G_def extended_order.elims(3) by fastforce
  next 
    case False 
    then have F0:" Min\<^bsub>G\<^esub> (val x) (val y) = (val x)" 
      using G_ord(1) True val_def by auto 
    have F1: "val (x \<oplus>\<^bsub>Q\<^sub>p\<^esub> y) = val x"
      using False True Qp_is_field field_def assms(1) 
        cring.cring_simprules(16) domain.axioms(1) by fastforce 
    then show ?thesis 
      using F0 F1 by (simp add: False G_ord(3) val_def) 
  qed
next
  case False 
    have Px: "x \<in> nonzero Q\<^sub>p"
      using False assms(1) nonzero_def by fastforce
    ...
    next
      ...
      have Pxy: "x \<oplus>\<^bsub>Q\<^sub>p\<^esub> y \<in> nonzero Q\<^sub>p" 
        by (simp add: False Qp_is_field assms(1) 
            assms(2) cring.cring_simprules(1) fieldE(1) nonzero_def)
      then show ?thesis using Px Py val_ultrametric0 by auto
  qed
qed
...
proof-
  ...
  have Cn: "c \<otimes>c \<in> nonzero Z\<^sub>p" 
    by (simp add: C Localization.submonoid.m_closed Zp_is_domain domain.nonzero_is_submonoid)
  ...
  have Ordxy1: "ord (x \<otimes>\<^bsub>Q\<^sub>p\<^esub> y) = (ord_Zp a) + (ord_Zp b) - ((ord_Zp c) + (ord_Zp c))" 
    by (simp add: An Bn C Ordxy0 ord_Zp_mult)
  then show ?thesis 
...
proof-
  have 0: "(val x) = *(ord x)*" 
    using assms(1) val_ord by auto 
  have 1: "(val y) = *(ord y)*" 
    using assms(2) val_ord by auto 
  have "(x \<otimes>\<^bsub>Q\<^sub>p\<^esub> y) \<noteq> \<zero>\<^bsub>Q\<^sub>p\<^esub>"
    using Qp_is_field assms(1) assms(2) integral nonzero_def by fastforce
...
proof(cases "x = \<zero>\<^bsub>Q\<^sub>p\<^esub> \<or> y = \<zero>\<^bsub>Q\<^sub>p\<^esub>")
  ...
  then show ?thesis 
    using G_mult(1) G_mult(2) Qp_is_field assms(1) assms(2) 
      domain.integral_iff field.axioms(1) val_def by fastforce
next
  case False
  have Px: "x \<in> nonzero Q\<^sub>p" 
    using False assms(1) nonzero_def by force
  have Py: "y \<in> nonzero Q\<^sub>p" 
    using False assms(2) nonzero_def by force
  then show ?thesis 
    by (simp add: Px)
...
proof-
  have "\<iota> x = frac x \<one>"
    using assms(1) by (simp add: local.inc_def Zp_nonzero_def(1))
  then have "ord ( \<iota> x) = ord_Zp x - ord_Zp \<one>"
    using assms(1) ord_of_frac  by (simp add: Zp_one_nonzero)
  then show ?thesis
    using ord_Zp_one  by simp
qed
...
proof(cases "x \<in> nonzero Z\<^sub>p")
  case True
  then show ?thesis 
    using inc_of_nonzero nonzero_def ord_Zp_def ord_of_inc val_Zp_def val_ord by force
next
  case False
  then show ?thesis 
...
"ord (\<pp> e (n::nat)) = n"
  using p_pow_nonzero ord_Zp_p ord_of_inc p_inc ord_Zp_p_pow p_natpow_inc by auto
...
proof-
  ...
  then have "ord_Zp a \<ge>0" 
    using A Zp_nonzero_def(2) ord_pos by auto
  then show ?thesis 
    by (simp add: A \<open>a \<in> nonzero Z\<^sub>p\<close> ord_of_inc)
qed
